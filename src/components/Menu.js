...
function Menu() {
  const foodItem = [myImg1, myImg2, myImg3];
  const [currentIdx, setCurrentIdx] = useState(0);
        ...
        <div style={{
          ...
          {Array.isArray(currentData) ? (
            currentData.length > 0 && typeof currentData[0] === 'object' && currentData[0].image ? (
              <div style={gridStyle}>
                {currentData.map((item, index) => (
                  <div key={index} style={foodCardStyle}>
                    <img 
                      src={item.image}
                      style={{
                        width: '100%',
      ...
      <div style={footerStyle}>
        <div style={footerSectionStyle}>
          ...
          <p style={footerTextStyle}>
            <span style={{ fontSize: isMobile ? '16px' : '20px', display: 'block' }}>ORDER NOW</span>
            +91-8649282653
          </p>
          {!isMobile && <span style={footerIconStyle}>📞</span>}
        </div>
        ...
        <div style={footerSectionStyle}>
          {isMobile && <span style={footerIconStyle}>📧</span>}
          <p style={footerTextStyle}>
            <span style={{ fontSize: isMobile ? '16px' : '20px', display: 'block' }}>NEED SUPPORT</span>
            Ready To Help
          </p>
          {!isMobile && <span style={footerIconStyle}>📧</span>}
        </div>
